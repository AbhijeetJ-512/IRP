...
from evaluate import eval_iou_2, onehot_encoding
...
def train(args):
    ...
    logging.basicConfig(
        filename=os.path.join(args.logdir, "results.log"),
        filemode="w",
    ...
    train_loader, val_loader = semantic_dataset(
        ...
        args.bsz,
        args.nworkers,
        depth_downsample_factor=args.depth_downsample_factor,
        depth_sup=args.depth_sup,
        use_depth_enc=args.use_depth_enc,
        use_depth_enc_bin=args.use_depth_enc_bin,
        add_depth_channel=args.add_depth_channel,
        use_lidar_10=True,
    ...
    model = get_model(
        ...
        args.direction_pred,
        args.angle_class,
        downsample=args.depth_downsample_factor,
        use_depth_enc=args.use_depth_enc,
    ...
    loss_fn = SimpleLoss(args.pos_weight).cpu()
    embedded_loss_fn = DiscriminativeLoss(
        args.embedding_dim, args.delta_v, args.delta_d
    ).cpu()
    ...
    for epoch in range(starting_epoch, args.nepochs):
        ...
        for batchi, (
            ...
            semantic, embedding, direction, depth = model(
                imgs.cpu(),
                trans.cpu(),
                rots.cpu(),
                intrins.cpu(),
                ...
                car_trans.cpu(),
                yaw_pitch_roll.cpu(),
                final_depth_map_bin_enc.cpu(),
                projected_depth.cpu(),
            )

            semantic_gt = semantic_gt.cpu().float()
            instance_gt = instance_gt.cpu()
            if args.depth_sup:
                final_depth_map = final_depth_map.cpu()
            ...
            final_loss = (
                seg_loss * args.scale_seg
                + var_loss * args.scale_var
                + dist_loss * args.scale_dist
                + direction_loss * args.scale_direction
                + depth_loss * args.scale_depth
            )
            final_loss.backward()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--xbound", nargs=3, type=float, default=[-90.0, 90.0, 0.15])
    parser.add_argument("--ybound", nargs=3, type=float, default=[-15.0, 15.0, 0.15])
    parser.add_argument("--zbound", nargs=3, type=float, default=[-10.0, 10.0, 20.0])
    parser.add_argument("--zgrid", nargs=3, type=float, default=[-3.0, 1.5, 0.15])
    parser.add_argument("--dbound", nargs=3, type=float, default=[2.0, 90.0, 1.0])
