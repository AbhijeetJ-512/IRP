...
def vis_vector(model, val_loader, angle_class):
    ...
    with torch.no_grad():
        for batchi, (
            ...
            segmentation, embedding, direction, depth = model(
                imgs.cpu(),
                trans.cpu(),
                rots.cpu(),
                intrins.cpu(),
...
if __name__ == "__main__":
    ...
    # nuScenes config
    parser.add_argument(
        "--dataroot", type=str, default="/media/hao/HaoData/dataset/nuScenes/"
    )
    parser.add_argument(
        "--version",
        type=str,
