...
def export_to_json(model, val_loader, angle_class, args):
    ...
    with torch.no_grad():
        for batchi, (imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, segmentation_gt, instance_gt, direction_gt, final_depth_map, final_depth_map_bin_enc, projected_depth) in enumerate(tqdm.tqdm(val_loader)):
            ...
            #     segmentation, embedding, direction = model(imgs.cpu(), trans.cpu(), rots.cpu(), intrins.cpu(),
            #                                             post_trans.cpu(), post_rots.cpu(), lidar_data.cpu(),
            #                                             lidar_mask.cpu(), car_trans.cpu(), yaw_pitch_roll.cpu())
            # else:
            segmentation, embedding, direction, _ = model(imgs.cpu(), trans.cpu(), rots.cpu(), intrins.cpu(),
                                                    post_trans.cpu(), post_rots.cpu(), lidar_data.cpu(),
                                                    lidar_mask.cpu(), car_trans.cpu(), yaw_pitch_roll.cpu(), final_depth_map_bin_enc.cpu(), projected_depth.cpu())
