...
def get_val_info(args):
    ...
    total_CD1 = torch.zeros(args.max_channel).cpu()
    total_CD2 = torch.zeros(args.max_channel).cpu()
    total_CD_num1 = torch.zeros(args.max_channel).cpu()
    total_CD_num2 = torch.zeros(args.max_channel).cpu()
    total_intersect = torch.zeros(args.max_channel).cpu()
    total_union = torch.zeros(args.max_channel).cpu()
    AP_matrix = torch.zeros((args.max_channel, len(THRESHOLDS))).cpu()
    AP_count_matrix = torch.zeros((args.max_channel, len(THRESHOLDS))).cpu()

    total_CD1_30_60 = torch.zeros(args.max_channel).cpu()
    total_CD2_30_60 = torch.zeros(args.max_channel).cpu()
    total_CD_num1_30_60 = torch.zeros(args.max_channel).cpu()
    total_CD_num2_30_60 = torch.zeros(args.max_channel).cpu()
    total_intersect_30_60 = torch.zeros(args.max_channel).cpu()
    total_union_30_60 = torch.zeros(args.max_channel).cpu()
    AP_matrix_30_60 = torch.zeros((args.max_channel, len(THRESHOLDS))).cpu()
    AP_count_matrix_30_60 = torch.zeros((args.max_channel, len(THRESHOLDS))).cpu()

    total_CD1_60_90 = torch.zeros(args.max_channel).cpu()
    total_CD2_60_90 = torch.zeros(args.max_channel).cpu()
    total_CD_num1_60_90 = torch.zeros(args.max_channel).cpu()
    total_CD_num2_60_90 = torch.zeros(args.max_channel).cpu()
    total_intersect_60_90 = torch.zeros(args.max_channel).cpu()
    total_union_60_90 = torch.zeros(args.max_channel).cpu()
    AP_matrix_60_90 = torch.zeros((args.max_channel, len(THRESHOLDS))).cpu()
    AP_count_matrix_60_90 = torch.zeros((args.max_channel, len(THRESHOLDS))).cpu()

    print('running eval...')
    for pred_map, confidence_level, gt_map in tqdm.tqdm(data_loader):
        # iou
        pred_map = pred_map.cpu() # torch.Size([4, 3, 200, 400])
        confidence_level = confidence_level.cpu()
        gt_map = gt_map.cpu()
        ...
        #total_intersect_30_60 += intersect.cpu()
        #total_union_30_60 += union.cpu()
        total_CD1_30_60 += CD1
        total_CD2_30_60 += CD2
        ...
        #total_intersect_60_90 += intersect.cpu()
        #total_union_60_90 += union.cpu()
        total_CD1_60_90 += CD1
        total_CD2_60_90 += CD2
