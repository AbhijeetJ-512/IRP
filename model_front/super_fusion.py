...
class CamEncode(nn.Module):
    def __init__(
        self, D, C, use_depth_enc=False, pretrained=True, add_depth_channel=False
    ):
    ...
    def forward(self, x, depth_enc, projected_depth):
        depth, x, image_features_out = self.get_depth_feat(
            x, depth_enc, projected_depth
        )
...
class LiftSplatShoot(nn.Module):
    ...
    def get_voxels(
        self, x, rots, trans, intrins, post_rots, post_trans, depth_enc, projected_depth
    ):
    ...
    def forward(
        ...
        x, depth, image_features_out = self.get_voxels(
            x, rots, trans, intrins, post_rots, post_trans, depth_enc, projected_depth
        )
...
class LidarPred(nn.Module):
    def __init__(
        self, use_cross=False, num_heads=1, pos_emd=True, neck_dim=256, cross_dim=256
    ):
        ...
        if self.use_cross:
            ...
            if pos_emd:
                self.pe_lidar = positionalencoding2d(neck_dim, 25 * 2, 75 * 2).cpu()
                self.pe_img = positionalencoding2d(neck_dim, 32, 88).cpu()
            self.multihead_attn = nn.MultiheadAttention(
                neck_dim, num_heads, dropout=0.3, batch_first=True
            )
            self.conv = nn.Sequential(
                nn.Conv2d(neck_dim + cross_dim, neck_dim, kernel_size=3, padding=1),
                nn.BatchNorm2d(neck_dim),
                nn.ReLU(inplace=True),
            )
            self.conv_cross = nn.Sequential(
                nn.Conv2d(neck_dim, cross_dim, kernel_size=1),
    ...
    def cross_attention(self, x, img_feature):
        ...
        attn_output, attn_output_weights = self.multihead_attn(query, key, key)
...
class AlignFAnew(nn.Module):
    ...
    def forward(self, low_stage, high_stage):
        ...
        low_stage = self.bilinear_interpolate_torch_gridsample2(
...
class SuperFusion(nn.Module):
    def __init__(
        ...
        self.lidar_pred = LidarPred(
            use_cross=True, num_heads=4, neck_dim=256, cross_dim=256
        )
    ...
    def forward(
        ...
        topdown, depth, image_features_out = self.lss(
            img, rots, trans, intrins, post_rots, post_trans, depth_enc, projected_depth
        )
        # print(topdown.shape)

        if self.lidar:
            lidar_feature, neck_feature = self.pp(lidar_data, lidar_mask)
