...
def sort_points_by_dist(coords):
    ...
    direction_matrix = diff_matrix / (
        dist_matrix.reshape(num_points, num_points, 1) + 1e-6
    )
    ...
    for i in range(num_points - 1):
        ...
        dist_metric = dist_matrix[last_idx] - 0 * (
            last_direction * direction_matrix[last_idx]
        ).sum(-1)
...
def connect_by_step(
    coords, direction_mask, sorted_points, taken_direction, step=5, per_deg=10
):
    while True:
        ...
        vector_to_target = step * np.array(
        ...
        target_point = np.array(
            [last_point[0] + vector_to_target[0], last_point[1] + vector_to_target[1]]
        )
...
def connect_by_direction(coords, direction_mask, step=5, per_deg=10):
    ...
    connect_by_step(
        coords, direction_mask, sorted_points, taken_direction, step, per_deg
    )
